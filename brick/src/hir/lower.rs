...
fn lower_coroutine(
    ...
    let generator_ty =
        ExpressionType::Pointer(PointerKind::UniqueRef, Box::new(func_ty.returns.clone()));
...
fn lower_function(
    ...
    if let Some(self_var_id) = func.self_var_id {
        instructions.push(HirNode::autogenerated(
            HirNodeValue::Parameter(parameter_offset, self_var_id),
            func_ty.params[0].clone(),
        ));
        parameter_offset += 1;
    }
    instructions.extend(
        func.func
            ...
            .map(|(i, (id, _param))| {
                HirNode::autogenerated(
                    HirNodeValue::Parameter(i + parameter_offset, *id),
                    func_ty.params[i + parameter_offset].clone(),
                )
            }),
    );
    let mut body = lower_node(decls, ast, ast.get(func.func.body));
...
pub fn lower_node(decls: &DeclarationContext, ast: &AstArena, node: &AstNode) -> HirNode {
    let value = match &node.value {
        ...
        AstNodeValue::TakePointer(kind, inner) => {
            HirNodeValue::TakePointer(*kind, lower_node_alloc(decls, ast, ast.get(*inner)))
        }
                    ...
                    let seq = vec![
                        ...
                        HirNode::autogenerated(
                            HirNodeValue::Assignment(
                                ...
                                if let ExpressionType::Pointer(ptr_kind, _) = match_decl_ty {
                                    ...
                                    Box::new(HirNode::autogenerated(
                                        HirNodeValue::TakePointer(*ptr_kind, union_variant_node),
                                        variant_ty.clone(),
                                    ))
                                } else {
                                    Box::new(HirNode::autogenerated(
