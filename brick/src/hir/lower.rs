...
fn lower_node(decls: &HashMap<TypeID, &StaticDeclaration>, node: &AstNode<'_>) -> HirNode {
    let value = match &node.value {
        ...
        AstNodeValue::Match(match_decl) => {
            // TODO: generate temp variables if necessary
            let value = Box::new(HirNode::autogenerated(
                HirNodeValue::UnionTag(lower_node_alloc(decls, &match_decl.value)),
                ExpressionType::Primitive(PrimitiveType::PointerSize),
            ));
            let Some(StaticDeclaration::Union(union_decl)) = match_decl
                .value
                .ty
                .get()
                .and_then(|ty| ty.id().map(|id| &decls[id]))
            else {
                unreachable!()
            };
            // TODO: is the match statement inherently exhaustive?
            let cases = union_decl
                ...
                .map(|union_variant| {
                    ...
                    HirNode::autogenerated(
                        if let Some((case_decl, _variant_decl)) = matching_variant {
                            ...
                            HirNodeValue::Sequence(vec![
                                ...
                                // TODO: assign union value to variable ID
                                body,
                            ])
                        } else {
                            // TODO: case type
