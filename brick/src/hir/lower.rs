...
use crate::{
    ...
    typecheck::{
        find_func, fully_dereference, shallow_dereference, traverse_dots, CollectionType,
        ExpressionType, FuncType, PointerKind, PrimitiveType, StaticDeclaration, TypecheckedFile,
        TypecheckedFunction,
    },
};
...
fn lower_node(decls: &HashMap<TypeID, &StaticDeclaration>, node: &AstNode<'_>) -> HirNode {
    let value = match &node.value {
        ...
        AstNodeValue::Match(match_decl) => {
            ...
            let Some(StaticDeclaration::Union(union_decl)) =
                shallow_dereference(match_decl_ty).id().map(|id| &decls[id])
            else {
                unreachable!()
            };
