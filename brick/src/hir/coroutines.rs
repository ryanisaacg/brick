...
pub fn rewrite_generator_calls(module: &mut HirModule) {
    module.par_visit_mut(|_return_ty, node| {
        ...
        if !matches!(
            &generator.ty,
            ExpressionType::Pointer(PointerKind::UniqueRef, _)
        ) {
            generator = HirNode::autogenerated(
                HirNodeValue::TakePointer(PointerKind::UniqueRef, Box::new(generator)),
                ExpressionType::Pointer(PointerKind::UniqueRef, Box::new(gen_ty)),
            );
        }
