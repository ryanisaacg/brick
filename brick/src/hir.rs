...
impl HirNode {
    ...
    fn children_impl<'a>(
        ...
        match &self.value {
            ...
            HirNodeValue::TypeParameterCall(_, _, lhs, args) => {
                // TODO: param types
...
pub enum HirNodeValue {
    ...
    /// Call a function associated with a type parameter, to be monomorphized into a Call later
    TypeParameterCall(TypeID, String, Box<HirNode>, Vec<HirNode>),
...
impl HirNodeValue {
    pub fn lvalue_mut(&mut self) -> Option<&mut HirNode> {
        match self {
            HirNodeValue::Call { func: lvalue, .. }
            | HirNodeValue::TypeParameterCall(_, _, lvalue, _)
